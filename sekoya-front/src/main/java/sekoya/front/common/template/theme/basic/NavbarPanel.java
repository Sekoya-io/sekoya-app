package sekoya.front.common.template.theme.basic;
  ...
  public NavbarPanel(
    ...
    List<NavigationMenuItem> menuItems =
        mainNavSupplier.get().stream()
            .filter(
                mainNavItem -> {
                  return (mainNavItem.getPageLinkGenerator() != null
                          && Condition.visible(mainNavItem.linkHidingIfInvalid("navLink"))
                              .applies())
                      || Condition.componentsAnyVisible(
                              mainNavItem.getSubMenuItems().stream()
          ...
          protected void populateItem(ListItem<NavigationMenuItem> item) {
            ...
            List<NavigationMenuItem> subMenuItems =
                navItem.getSubMenuItems().stream()
                    .filter(
                        subMenuItem ->
                            Condition.visible(subMenuItem.linkHidingIfInvalid("navLink")).applies())
                    .collect(ImmutableList.toImmutableList());
                    ...
                    new CoreLabel(
                            ...
                            new IModel<String>() {
                              ...
                              public String getObject() {
                                return SekoyaSession.get().getOriginalAuthentication() != null
                                    ? SekoyaSession.get().getOriginalAuthentication().getName()
                                    : null;
                              }
                            })
                        .hideIfEmpty(),
    ...
    add(
        new AjaxLink<Void>("signInAsUndo") {
          ...
          public void onClick(AjaxRequestTarget target) {
            try {
              SekoyaSession.get().signInAsMe();
              SekoyaSession.get().success(getString("authentication.signInAs.undo.success"));
            } catch (Exception e) {
          ...
          protected void onConfigure() {
            super.onConfigure();
            setVisible(SekoyaSession.get().getOriginalAuthentication() != null);
          }
        },
