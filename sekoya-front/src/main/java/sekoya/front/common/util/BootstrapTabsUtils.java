...
public final class BootstrapTabsUtils {
  ...
  public static Component[] build(
      String markupId, Component tab, Component panel, IModel<Boolean> activeModel) {
    return build(getTabMarkupId(markupId), markupId, tab, panel, activeModel);
  }

  public static Component[] build(
      String tabMarkupId,
      String panelMarkupId,
      Component tab,
      Component panel,
      IModel<Boolean> activeModel) {
    ...
    panel.add(
        new ClassAttributeAppender(
            Condition.isTrue(activeModel).then("active show").otherwise("")));
