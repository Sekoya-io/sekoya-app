package sekoya.front.user.popup;
...
public class UserAdministrateurFonctionnelSavePopup extends AbstractAjaxModalPopupPanel<User> {

  private static final long serialVersionUID = 1L;

  private static final Logger LOGGER =
      LoggerFactory.getLogger(UserAdministrateurFonctionnelSavePopup.class);
  ...
  public UserAdministrateurFonctionnelSavePopup(String id) {
  ...
  protected Component createBody(String wicketId) {
    ...
    boolean passwordRequired =
        securityManagementcontrollerService
                .getSecurityOptions(UserType.ADMINISTRATEUR_TECHNIQUE)
  ...
  protected Component createFooter(String wicketId) {
    DelegatedMarkupPanel footer =
        new DelegatedMarkupPanel(wicketId, UserAdministrateurFonctionnelSavePopup.class);
    ...
    footer.add(
        new AjaxButton("save", form) {
          ...
          protected void onSubmit(AjaxRequestTarget target) {
            try {
              IModel<User> userModel = UserAdministrateurFonctionnelSavePopup.this.getModel();
              User user = userModel.getObject();
              User authenticatedUser = SekoyaSession.get().getUser();
              String password = passwordModel.getObject();

              userControllerService.saveUserAdministrateurFonctionnel(user, password);
              ...
              if (addModeCondition().applies() && !StringUtils.hasText(password)) {
                ...
                throw UserAdministrateurFonctionnelDetailPage.MAPPER
                    .ignoreParameter2()
                    .map(userModel)
                ...
                if (authenticatedUser != null
                    ...
                    && user.getLocale() != null) {
                  SekoyaSession.get().setLocale(user.getLocale());
                }
                closePopup(target);
                target.add(getPage());
              }
