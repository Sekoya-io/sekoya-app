...
public abstract class AbstractGenericPermissionEvaluator<T>
  ...
  @Autowired protected ISekoyaSecurityService securityService;
  ...
  @VisibleForTesting // abstract methods not visible from mockito since V5.4.0
  public final boolean hasPermission(IUser user, String permissionName) {
    return securityService.hasPermission(user, permissionFactory.buildFromName(permissionName));
  }

  @VisibleForTesting // abstract methods not visible from mockito since V5.4.0
  public final boolean hasPermission(IUser user, Object securedObject, String permissionName) {
    return securityService.hasPermission(
        user, securedObject, permissionFactory.buildFromName(permissionName));
  }

  @VisibleForTesting // abstract methods not visible from mockito since V5.4.0
  public final boolean hasRole(IUser user, String role) {
