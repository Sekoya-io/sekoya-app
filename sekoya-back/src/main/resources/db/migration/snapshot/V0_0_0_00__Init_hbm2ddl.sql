...
create table commune_codepostal (Commune_id bigint not null, codepostal varchar(5));
...
create table Site (id bigint not null, adresse_adresse1 varchar(255) not null, adresse_adresse2 varchar(255), adresse_codePostal varchar(5) not null, chiffreAffaires integer, creation_date timestamp(6) with time zone not null, creation_subject_label text, creation_subject_reference_id bigint, creation_subject_reference_type varchar(255), creation_subject_serialized text, enabled boolean not null, latitude numeric(8,6) not null, longitude numeric(9,6) not null, modification_date timestamp(6) with time zone not null, modification_subject_label text, modification_subject_reference_id bigint, modification_subject_reference_type varchar(255), modification_subject_serialized text, nom varchar(255) not null, typologie varchar(255) not null check ((typologie in ('TYPOLOGIE1','TYPOLOGIE2'))), adresse_commune_id bigint not null, organisation_id bigint not null, primary key (id));
...
create table UserOrganisation (user_id bigint not null, organisation_id bigint not null, primary key (user_id));
...
alter table if exists UserOrganisation add constraint FKq6ap8umlm8fxigy0ju2ol0c8y foreign key (user_id) references user_;
